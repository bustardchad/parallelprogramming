/* Chad Bustard, parallel finite difference method using the most naive implementation of the Gauss-Seidel Method with two for loops both parallelized, CS759 Final Project */
...
//Timing template provided by Tim Haines
...
//set up openmp things:
////////////////////////////////
    int num = 8; //number of threads
    omp_set_num_threads(num);
    omp_lock_t maximum_lock;
    omp_init_lock (&maximum_lock);
////////////////////////////////
//
...
ofstream fout1("initParallel.txt");
...
ofstream fout("uFinal_parallel1.txt");

        if (fout) {
         cout << "Writing final u(x,y) to file 'uFinal_parallel1.txt'" << endl;
         for (int i = 0; i < N + 1; i++) {
             for (int j = 0; j < N + 1; j++) {
                 fout << u[i][j] << " ";
             }
